...
# 
# A = S
# valor_umbral = 0.2 * np.max(S)
# A[A < valor_umbral] = 0
# alpha = 0.001
# 
# pwdd = 'H:/Unidades compartidas/TF-Andres/Graficos_Finales/Correcciones/'
# 
# # =============================================================================
...
# #plt.savefig(pwdd+"Mapa_DT2_Dodecano_magma_para_surf_600", dpi=600, bbox_inches='tight')
# plt.show()
# =============================================================================
...
# 
# 
# pwdd = 'H:/Unidades compartidas/TF-Andres/2024-Dif-T1/Graficos/'
# 
...
# ax.contour(T2_Oct[:,0], D_Oct[:,0], A_Oct, 10, vmax=0.004, cmap= 'Wistia_r', zorder=-2)
...
# plt.tick_params(axis='both', which='major', labelsize=10)
# #plt.savefig(pwdd+"DT2_sin_agua_DodeArriba", dpi=600, bbox_inches='tight')
# plt.show()
# =============================================================================
...
pwdNOct = 'H:/Unidades compartidas/TF-Andres/Mediciones/Mediciones finales/T1D_Todos/Octano/'

S_NOct = pd.read_csv(pwdNOct+"Transformada.txt", header=None).to_numpy()
T1_NOct = pd.read_csv(pwdNOct+"VectorT1.txt", header=None).to_numpy()
D_NOct = pd.read_csv(pwdNOct+"VectorD.txt", header=None).to_numpy()
...
A_NOct = S_NOct/np.max(S_NOct)
valor_umbral = 0.1
A_NOct[A_NOct < valor_umbral] = 0
...
ax.contour(T1_dode[:,0], D_dode[:,0], A_dode.T, 8, cmap= 'Reds_r', zorder=-1)
ax.contour(T1_NOct[:,0], D_NOct[:,0], A_NOct.T, 8, cmap= 'Oranges_r', zorder=3)
ax.contour(T1_agua[:,0], D_agua[:,0], A_agua.T, 8, cmap= 'Blues_r')
ax.contour(T1_hept[:,0], D_hept[:,0], A_hept.T, 8, cmap= 'Greens_r', zorder=2)
...
#plt.savefig(pwdd+"Nuevo_con_todos", dpi=600, bbox_inches='tight')
plt.show()
